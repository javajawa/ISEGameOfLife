...
public class LoansInfo extends JPanel implements Plugin
{
			...
			for (Map.Entry<String, LoansGroup> entry : p_players.entrySet())
			{
				while (iter.hasNext())
				{
					...
					if (p_players.get(id) != null)
					{
						...
						data.add("Greediness: " + LoansGroup.getGreediness(
										p_players.get(id).getDataModel()));
						data.add("++++++LOAN HISTORY OF THIS GROUP++++++");

						//Display debtors / loans given
						data.add("Debtors History-------------------------------");
						Map<String, List<Tuple<Double, Double>>> loansGiven = LoansGroup.getLoansGiven(p_players.get(
										id).getDataModel());
						if (loansGiven != null)
						{
						...
						//Display debtors / loans given
						Map<String, List<Tuple<Double, Double>>> loansTaken = LoansGroup.getLoansTaken(p_players.get(
										id).getDataModel());
						if (loansTaken != null)
						{
	...
	public void onSimulationComplete()
	{
		...
		data.add(
						"*Note: Amounts given and amounts borrowed may not balance because dead groups are not displayed in this summary");
	}
